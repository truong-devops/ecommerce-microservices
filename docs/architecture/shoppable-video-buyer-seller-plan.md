...
| Seller | Publish/unpublish | Seller cần tắt video khi hết hàng/sai nội dung |
...
| `product-service` | NestJS + Mongoose, domain hiện có `products`, `shop-decor` | Nên thêm module/schema video trong product-service |
...
| `seller` | Đã có skeleton sidebar/page video | Cần nối upload/create/publish API thật |
...
### 6.1 Seller tạo và publish video
...
10. Seller bấm `Gửi duyệt` hoặc `Publish` nếu môi trường dev cho phép bypass review.
11. `product-service` validate sản phẩm, status, media rồi chuyển trạng thái.
12. Khi published, service phát event để analytics/feed cập nhật.
...
Phase đầu có thể dùng config `VIDEO_REVIEW_REQUIRED=false` để seller publish trực tiếp trong dev. Nhưng model/API vẫn nên chuẩn bị status review để không phải sửa lại sau.
...
| `processing` | Đã có media key, đang validate metadata/file | Seller | `review_pending`, `published`, `processing_failed` |
...
| `hidden` | Seller/admin tạm ẩn | Seller, Moderator | `published`, `archived` |
...
| `POST` | `/videos/:videoId/publish` | `SELLER` owner | Publish trực tiếp nếu review bypass |
...
| `product-service` | `npm --workspace services/product-service run test` | DTO validation, owner policy, state transition, feed filter |
...
|---|---|---|
| `buyer-web` | `npm --prefix frontend/apps/buyer-web run lint` | Header/video page/home section |
| `seller` | `npm --prefix frontend/apps/seller run lint` | Sidebar/video management page |
| `moderator` | `npm --prefix frontend/apps/moderator run lint` | Review queue nếu làm |
...
| Task ID | Deliverable | Files/Scope | Status | % | Evidence | Next step | Blocker |
|---|---|---|---|---:|---|---|---|
| `VID-BE-01` | Thêm schema `product_videos` | `services/product-service/src/modules/products/entities` hoặc module `videos` | DONE | 100 | File/Build | Theo dõi analytics phase sau | None |
| `VID-BE-02` | DTO create/update/confirm media | `services/product-service/src/modules/.../dto` | DONE | 100 | File/Build | Bổ sung moderation DTO sau | None |
...
- Seller tạo draft, confirm media, gắn product, publish được qua API.
...
| `VID-GW-03` | Route moderation private | `api-gateway` | TODO | 0 | Test | Chỉ moderator/admin | Phase moderation |

Definition of Done:

- Buyer gọi feed qua gateway được.
- Seller gọi create/publish qua gateway được với token.
...
- Seller upload và publish được từ UI.
...
| Task ID | Deliverable | Files/Scope | Status | % | Evidence | Next step | Blocker |
|---|---|---|---|---:|---|---|---|
| `VID-BUYER-01` | API client feed video | `frontend/apps/buyer-web/src/lib` | TODO | 0 | File | Fetch feed qua BFF/API route | Gateway chưa xong |
| `VID-BUYER-02` | Home section dùng API thật | `VideoHighlightsSection.tsx` | TODO | 0 | Demo | Replace mock | Feed API |
| `VID-BUYER-03` | `/videos` feed dùng API thật | `frontend/apps/buyer-web/src/app/videos/page.tsx` | TODO | 0 | Demo | Render videos/products | Feed API |
| `VID-BUYER-04` | Tracking view/click | `buyer-web` | TODO | 0 | API/Demo | Debounce + idempotency | Event API |
| `VID-BUYER-05` | Product click/add-to-cart flow | `buyer-web` | TODO | 0 | Demo | Navigate/cart | Product/cart APIs |
...
| Task ID | Deliverable | Files/Scope | Status | % | Evidence | Next step | Blocker |
|---|---|---|---|---:|---|---|---|
| `VID-AN-01` | Event types video | `shared/kafka/events`, `shared/kafka/topics` hoặc analytics constants | TODO | 0 | File | Chốt topic hiện dùng | None |
| `VID-AN-02` | Ingest raw events | `services/analytics-service` | TODO | 0 | Test | Store/dedupe event | Kafka config |
| `VID-AN-03` | Aggregate KPI | `analytics-service` | TODO | 0 | Test | View/click/add-to-cart rate | Raw events |
| `VID-AN-04` | Seller analytics endpoint | `product-service` hoặc `analytics-service` | TODO | 0 | API | Decide query owner | Analytics aggregate |
| `VID-AN-05` | KPI display in seller UI | `frontend/apps/seller` | TODO | 0 | Demo | Connect cards/table | Endpoint |
...
| Task ID | Deliverable | Files/Scope | Status | % | Evidence | Next step | Blocker |
|---|---|---|---|---:|---|---|---|
| `VID-MOD-01` | Review queue API | `product-service` | TODO | 0 | API/Test | Query review_pending | Core API |
| `VID-MOD-02` | Approve/reject API | `product-service` | TODO | 0 | API/Test | Status transitions | Core API |
| `VID-MOD-03` | Moderator UI | `frontend/apps/moderator` | TODO | 0 | Demo | Review page | APIs |
| `VID-HARD-01` | Idempotency publish/events | Backend services | TODO | 0 | Test | Dedupe keys | Core flows |
| `VID-HARD-02` | Rate limit event APIs | Gateway/service | TODO | 0 | Test | Avoid event spam | API stable |
| `VID-HARD-03` | Cleanup orphan media | media/product-service job or docs | TODO | 0 | File/Test | Delete/archive strategy | Optional |
...
| Task ID | Deliverable | Files/Scope | Status | % | Evidence | Next step | Blocker |
|---|---|---|---|---:|---|---|---|
| `VID-DEMO-01` | Seed/demo data | `product-service`, docs | TODO | 0 | Demo | Seller/product/video sample | API stable |
| `VID-DEMO-02` | Demo script 8-10 phút | `docs/architecture` | TODO | 0 | File | Viết kịch bản | Flow stable |
| `VID-DEMO-03` | Troubleshooting guide | `docs/architecture` | TODO | 0 | File | Lỗi upload/video/CORS | Flow stable |
...
| 2026-05-15 | `VID-BE-01..08` | Thêm domain `product_videos`, seller APIs, buyer feed APIs, state machine, unit test | `services/product-service/src/modules/products/**` | API/DB | `npm --workspace services/product-service run test`; `npm --workspace services/product-service run build` | DONE | `VID-GW-01` | None |
...
| 2026-05-15 | Cho phép review bypass ở dev | Demo nhanh hơn | Không chặn seller flow | Dễ quên bật review | Config rõ `VIDEO_REVIEW_REQUIRED` |
...
1. Seller tạo video từ UI, upload `mp4/webm`, gắn sản phẩm, publish được.
2. Buyer mở `/videos` thấy video từ backend thật.
3. Buyer click product trong video đi đúng product/cart flow.
4. View/click/add-to-cart được ghi nhận vào analytics.
5. Seller thấy KPI cơ bản cho từng video.
6. Gateway route và auth policy rõ public/private.
7. Có tests cho media policy, product video state machine, gateway route.
8. Có demo script và dữ liệu demo chạy lại được.
