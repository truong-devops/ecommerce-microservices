...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!MODERATOR_ROLES.has(exchanged.user.role)) {
      loginUrl.searchParams.set('oauthError', 'Account role is not allowed for moderator dashboard');
      return NextResponse.redirect(loginUrl);
    }
    ...
    const html = renderBridgeHtml({
      storageKey: 'moderator_auth_session',
      session,
      returnUrl
    });
