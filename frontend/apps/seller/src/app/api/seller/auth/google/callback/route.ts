...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!DASHBOARD_ROLES.has(exchanged.user.role)) {
      loginUrl.searchParams.set('oauthError', 'Account role is not allowed for seller dashboard');
      return NextResponse.redirect(loginUrl);
    }
    ...
    const html = renderBridgeHtml({
      storageKey: 'seller_auth_session',
      session,
      returnUrl
    });
