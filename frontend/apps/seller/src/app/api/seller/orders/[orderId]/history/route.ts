import type { SellerOrderStatusHistoryOutput } from '@/lib/api/types';
...
export async function GET(request: Request, context: RouteContext) {
  ...
  if (!ORDER_ROLES.has(claims.role)) {
    return fail(403, 'FORBIDDEN', 'Role is not allowed to get order history');
  }
  ...
  try {
    const history = await requestUpstream<SellerOrderStatusHistoryOutput>(
