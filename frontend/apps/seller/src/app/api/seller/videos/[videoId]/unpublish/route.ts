...
export async function POST(request: Request, context: { params: Promise<{ videoId: string }> }) {
  ...
  if (!VIDEO_EDIT_ROLES.has(claims.role)) {
    return fail(403, 'FORBIDDEN', 'Role is not allowed to unpublish videos');
  }
  ...
  try {
    const updated = await requestUpstream<unknown>(`${serviceBaseUrls.product}/videos/${videoId}/unpublish`, {
      method: 'POST',
      headers: {
