...
# Compare inventory-service (old NestJS) vs inventory-service-go performance.
# Env:
# BASE_URL_OLD default: http://localhost:3027/api/v1
# BASE_URL_GO  default: http://localhost:3037/api/v1
...
# SCENARIO     validate | stock | adjust

BASE_URL_OLD="${BASE_URL_OLD:-http://localhost:3027/api/v1}"
BASE_URL_GO="${BASE_URL_GO:-http://localhost:3037/api/v1}"
...
SKU="${SKU:-BENCH-INV-$(date +%s)}"

if [[ "$SCENARIO" != "validate" && "$SCENARIO" != "stock" && "$SCENARIO" != "adjust" ]]; then
  echo "SCENARIO must be one of: validate | stock | adjust" >&2
  exit 1
fi

node - "$BASE_URL_OLD" "$BASE_URL_GO" "$JWT_SECRET" "$CONCURRENCY" "$DURATION_SEC" "$TIMEOUT_MS" "$SCENARIO" "$SKU" <<'NODE'
const crypto = require('crypto');
const { performance } = require('perf_hooks');

const [,, baseOld, baseGo, jwtSecret, concurrencyRaw, durationRaw, timeoutRaw, scenario, sku] = process.argv;
...
async function ensureStock(baseUrl) {
  const payload = {
  ...
  await fetch(`${baseUrl}/inventory/stocks/${sku}/adjust`, {
    method: 'PATCH',
    headers: {
...
async function requestOnce(baseUrl) {
  ...
  let url = `${baseUrl}/inventory/validate?sku=${encodeURIComponent(sku)}&quantity=1`;
  let method = 'GET';
  let body;

  if (scenario === 'stock') {
    url = `${baseUrl}/inventory/stocks/${encodeURIComponent(sku)}`;
    headers.authorization = `Bearer ${sellerToken}`;
  } else if (scenario === 'adjust') {
    method = 'PATCH';
    url = `${baseUrl}/inventory/stocks/${encodeURIComponent(sku)}/adjust`;
    headers.authorization = `Bearer ${sellerToken}`;
    headers['content-type'] = 'application/json';
    body = JSON.stringify({ deltaOnHand: 1, reason: `bench-${seq++}` });
  }
  ...
  try {
    ...
    const ok = res.status === 200 || (scenario === 'adjust' && res.status === 201);
...
(async () => {
  ...
  await ensureStock(baseOld);
  await ensureStock(baseGo);
  await Promise.all([requestOnce(baseOld), requestOnce(baseGo)]);

  const oldResult = await runBenchmark('legacy inventory-service (NestJS)', baseOld);
  const goResult = await runBenchmark('inventory-service-go', baseGo);
