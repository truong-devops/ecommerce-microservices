...
BASE_URL_OLD="${BASE_URL_OLD:-http://localhost:3026/api/v1}"
BASE_URL_GO="${BASE_URL_GO:-http://localhost:3036/api/v1}"
...
async function requestOnce(baseUrl) {
  ...
  if (scenario === 'post') {
    ...
    body = JSON.stringify(paymentPayload(seq));
  }
...
(async () => {
  ...
  const oldResult = await runBenchmark('legacy payment-service (NestJS)', baseOld);
  const goResult = await runBenchmark('payment-service-go', baseGo);
