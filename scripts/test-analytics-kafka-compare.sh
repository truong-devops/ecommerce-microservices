...
COMPOSE_BASE="services/analytics-service-nest/docker-compose.compare.yml"
COMPOSE_KAFKA="services/analytics-service-nest/docker-compose.compare.kafka.yml"
AUTO_UP="${AUTO_UP:-0}"
TOPIC="${TOPIC:-analytics.events}"
...
wait_log_contains() {
  ...
  while true; do
    if compose_kafka logs --since=5m "$service" 2>/dev/null | grep -q "$pattern"; then
      echo "$label ready"
      return 0
    fi
...
wait_log_contains analytics-service-old "Kafka consumer started" "legacy consumer"
wait_log_contains analytics-service-go "kafka consumer started" "go consumer"
sleep 2

MARKER="analytics-kafka-compare-$(date +%s)"
OCCURRED_AT="2026-01-01T00:00:00.000Z"
EVENT_JSON="$(node -e "console.log(JSON.stringify({eventType:'order.created',occurredAt:'$OCCURRED_AT',payload:{sellerId:'11111111-1111-4111-8111-111111111111',userId:'33333333-3333-4333-8333-333333333333',orderId:'$MARKER',status:'CREATED',amount:120000,currency:'VND'}}))")"
...
wait_for_dedupe() {
  ...
  for _ in $(seq 1 90); do
    local raw_count
    raw_count="$($fn "SELECT COUNT(*) FROM analytics_events_raw WHERE event_type = 'order.created' AND payload_json->>'orderId' = '$MARKER';" | tr -d '[:space:]')"

    if [[ "$raw_count" == "1" ]]; then
      echo "$label dedupe OK: analytics_events_raw=$raw_count"
      return 0
    fi
  ...
  local final_raw
  final_raw="$($fn "SELECT COUNT(*) FROM analytics_events_raw WHERE event_type = 'order.created' AND payload_json->>'orderId' = '$MARKER';" | tr -d '[:space:]')"
  echo "$label final count: analytics_events_raw=$final_raw" >&2
  return 1
}
...
echo
echo "Kafka dedupe checks passed on both analytics services"
