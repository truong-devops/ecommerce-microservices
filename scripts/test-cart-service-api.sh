...
function b64(input) { return Buffer.from(JSON.stringify(input)).toString('base64url'); }
const header = b64({ alg: 'HS256', typ: 'JWT' });
const payload = b64({ sub: userId, email, role, jti: `cart-test-jti-${Date.now()}`, iat: now, exp: now + 3600 });
...
wait_for_health() {
  ...
  while true; do
    ...
    if (( "$(date +%s)" - start >= timeout_sec )); then
      echo "Health check timeout. Last status=$status ($base_url/health)" >&2
      exit 1
    fi
    sleep 1
  done
...
print_step "Reset cart state"
call_api DELETE "$BASE_URL/cart" "" "$BUYER_TOKEN"
assert_status_in "200"
assert_success_true
echo "Reset cart OK"
...
echo
echo "All cart-service smoke tests passed"
