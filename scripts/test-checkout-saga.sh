...
WAREHOUSE_ID="${WAREHOUSE_ID:-44444444-4444-4444-8444-444444444444}"
...
make_token() {
  local user_id="$1" email="$2" role="$3"
...
payload = b64(json.dumps({"sub":user_id,"email":email,"role":role,"sessionId":"checkout-saga-session","tokenVersion":1,"jti":"checkout-saga-"+str(int(time.time()*1000)),"iat":int(time.time()),"exp":int(time.time())+3600}, separators=(",", ":")).encode())
...
discover_product() {
  call_api_url GET "$PRODUCT_BASE_URL/products"
  assert_status_in "200"; assert_success_true
  JSON_INPUT="$RESPONSE_BODY" python3 <<'PY'
import json, os, sys
...
call_api_url() {
  local method="$1" url="$2" data="${3:-}" bearer="${4:-}" idem="${5:-}"
  local tmp_file
  tmp_file="$(mktemp)"
  local -a cmd=(curl -sS -o "$tmp_file" -w "%{http_code}" -X "$method" "$url" -H "Accept: application/json")
  [[ -n "$bearer" ]] && cmd+=(-H "Authorization: Bearer $bearer")
  [[ -n "$idem" ]] && cmd+=(-H "Idempotency-Key: $idem")
  [[ -n "$data" ]] && cmd+=(-H "Content-Type: application/json" -d "$data")
  RESPONSE_STATUS="$("${cmd[@]}")" || { rm -f "$tmp_file"; echo "Request failed: $method $url" >&2; exit 1; }
...
assert_status_in() {
  local expected="$1"
  for status in $expected; do [[ "$RESPONSE_STATUS" == "$status" ]] && return; done
...
assert_success_true() {
  ...
  [[ "$success" == "True" || "$success" == "true" ]] || { echo "Expected success=true. Body: $RESPONSE_BODY" >&2; exit 1; }
}

wait_for_order_status() {
  local order_id="$1" expected="$2" token="$3"
  local deadline=$((SECONDS + SAGA_TIMEOUT_SECONDS))
  while (( SECONDS < deadline )); do
    call_api_url GET "$ORDER_BASE_URL/orders/$order_id" "" "$token"
...
wait_for_stock() {
  local sku="$1" expected_on_hand="$2" expected_reserved="$3" token="$4"
  local deadline=$((SECONDS + SAGA_TIMEOUT_SECONDS))
  while (( SECONDS < deadline )); do
    call_api_url GET "$INVENTORY_BASE_URL/inventory/stocks/$sku" "" "$token"
    if [[ "$RESPONSE_STATUS" == "200" ]]; then
      local on_hand reserved
      on_hand="$(json_field "$RESPONSE_BODY" data.onHand || true)"
      reserved="$(json_field "$RESPONSE_BODY" data.reserved || true)"
      [[ "$on_hand" == "$expected_on_hand" && "$reserved" == "$expected_reserved" ]] && return 0
    fi
    sleep 2
  done
  echo "Timed out waiting for stock $sku onHand=$expected_on_hand reserved=$expected_reserved" >&2
  echo "Last body: $RESPONSE_BODY" >&2
  exit 1
}
...
call_api_url PATCH "$INVENTORY_BASE_URL/inventory/stocks/$TEST_SKU/adjust" "{\"productId\":\"aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa\",\"sellerId\":\"$SELLER_ID\",\"deltaOnHand\":100,\"reason\":\"checkout saga happy path $RUN_ID\"}" "$SELLER_TOKEN"
assert_status_in "200"; assert_success_true
START_ON_HAND="$(json_field "$RESPONSE_BODY" data.onHand)"

print_step "Create order"
ORDER_PAYLOAD="{\"currency\":\"$ORDER_CURRENCY\",\"shippingAmount\":0,\"discountAmount\":0,\"note\":\"checkout saga happy $RUN_ID\",\"items\":[{\"productId\":\"$PRODUCT_ID\",\"sku\":\"$TEST_SKU\",\"productName\":\"Saga item\",\"quantity\":$QUANTITY,\"unitPrice\":$UNIT_PRICE}]}"
call_api_url POST "$ORDER_BASE_URL/orders" "$ORDER_PAYLOAD" "$CUSTOMER_TOKEN" "checkout-saga-$RUN_ID"
...
print_step "Wait for inventory reservation"
EXPECTED_RESERVED="$QUANTITY"
wait_for_stock "$TEST_SKU" "$START_ON_HAND" "$EXPECTED_RESERVED" "$SELLER_TOKEN"

print_step "Create mock captured payment"
PAYMENT_PAYLOAD="{\"orderId\":\"$ORDER_ID\",\"currency\":\"$ORDER_CURRENCY\",\"amount\":$TOTAL_AMOUNT,\"provider\":\"mock\",\"autoCapture\":true}"
call_api_url POST "$PAYMENT_BASE_URL/payments/intents" "$PAYMENT_PAYLOAD" "$CUSTOMER_TOKEN" "checkout-payment-$RUN_ID"
assert_status_in "200 201"; assert_success_true
PAYMENT_STATUS="$(json_field "$RESPONSE_BODY" data.status)"
[[ "$PAYMENT_STATUS" == "CAPTURED" ]] || { echo "Expected payment CAPTURED, got $PAYMENT_STATUS" >&2; exit 1; }

print_step "Wait for order CONFIRMED"
wait_for_order_status "$ORDER_ID" "CONFIRMED" "$CUSTOMER_TOKEN"

print_step "Wait for inventory confirmation"
EXPECTED_ON_HAND="$((START_ON_HAND - QUANTITY))"
wait_for_stock "$TEST_SKU" "$EXPECTED_ON_HAND" "0" "$SELLER_TOKEN"

echo
echo "Checkout saga happy path passed."
