...
func main() {
	...
	inventoryService := service.NewInventoryService(repo, logger, cfg.ReservationDefaultTTL, cfg.ReservationExpireBatch)
	healthService := service.NewHealthService(cfg.AppName, repo, redisService)

	inventoryHandler := handler.NewInventoryHandler(inventoryService)
	healthHandler := handler.NewHealthHandler(healthService)

	httpHandler := router.New(cfg, logger, redisService, inventoryHandler, healthHandler)
	...
	expirer := service.NewReservationExpirer(inventoryService, logger, cfg.ReservationExpireInterval)
	consumer := events.NewConsumer(cfg, logger, inventoryService)
	defer consumer.Close()

	runCtx, runCancel := context.WithCancel(context.Background())
	defer runCancel()
	go dispatcher.Run(runCtx)
	go expirer.Run(runCtx)
