...
import (
	...
	"product-service-go/internal/router"
...
func main() {
	...
	videoService := service.NewVideoService(videoRepo, redisService, cfg.MediaPublicBaseURL)
	...
	go func() {
		logger.Info("product-service-go started", zap.String("service", cfg.AppName), zap.Int("port", cfg.Port), zap.String("apiPrefix", cfg.APIPrefix))
