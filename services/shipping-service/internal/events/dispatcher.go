...
func NewDispatcher(repo *repository.ShippingRepository, publisher *Publisher, logger *zap.Logger, interval time.Duration, batchSize int, maxRetry int) *Dispatcher {
	return &Dispatcher{repo: repo, publisher: publisher, logger: logger, interval: interval, batchSize: batchSize, maxRetry: maxRetry}
}
